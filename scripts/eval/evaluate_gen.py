...
class Crystal(object):
    ...
    def __init__(self, crys_array_dict):
        ...
        # print('frac: ', self.frac_coords)
        # print('atom: ', self.atom_types)
    ...
    def get_match_rate_and_rms(self):
        ...
        for i in tqdm(range(len(self.preds[0]))):
            ...
            if len(tmp_rms_dists) == 0:
                ...
                print('valid rmsd: ', tmp_rms_dists)
...
def get_crystal_array_list(file_path, batch_idx=0, rank_selected=False, select_closest=False):
    ...
    if batch_idx == -1:
        ...
        if rank_selected:
            tmp = np.array(crys_array_list, dtype='object')
            ### (20, n_data)
            ref_energy = data['pred_energy']
            if select_closest:
                ### (n_data, 1)
                target_energy = data['target_energy']
                ### (20, n_data)
                ref_energy = torch.abs(target_energy.transpose(0, -1)-ref_energy)

            min_idx = torch.argmin(ref_energy, dim=0).numpy()
            crys_array_list = tmp[min_idx, range(tmp.shape[-1])].tolist()
...
def main(args):
    ...
    if 'opt' in args.tasks:
        ...
        crys_array_list, true_crystal_array_list = get_crystal_array_list(
            recon_file_path, batch_idx = batch_idx, rank_selected=args.rank_selected, select_closest=args.select_closest)
        if args.gt_file != '':
            print('*****************gt')
            csv = pd.read_csv(args.gt_file)
            gt_crys = p_map(get_gt_crys_ori, csv['cif'])
        ...
        if not args.multi_eval or args.rank_selected:
            print('*****************pred')
    ...
    if args.label == '':
        metrics_out_file = 'eval_metrics'
    else:
        metrics_out_file = f'eval_metrics_{args.label}'

    metrics_out_file = metrics_out_file + '_rank.json' if args.rank_selected else '.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rank_selected',action='store_true')
    parser.add_argument('--select_closest',action='store_true')
