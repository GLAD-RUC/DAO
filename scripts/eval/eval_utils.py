...
from dao.common.constants import CompScalerMeans, CompScalerStds
from dao.common.data_utils import StandardScaler, chemical_symbols
from dao.pl_modules.FTModels import CrystGenerativeFinetuneModel
from dao.pl_data.dataset import TensorCrystDataset
...
def load_from_checkpoint_torch(checkpoint_path, model, strict=True):
    ...
    for k, v in state_dict.items():
        if k.startswith('model.'): 
            print('state_dict_name starts with model.')
            name = k[6:]
        else:
            name = k
...
def get_model_path(eval_model_name):
    import dao
    model_path = (
        Path(dao.__file__).parent / 'prop_models' / eval_model_name)
    return model_path
