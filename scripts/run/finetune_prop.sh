...
# Note: This script keeps the original multi-block experiment configs used for paper reproduction.
# Before running, comment out the blocks you don't need, and update CKPT/GPU paths accordingly.
...
CUDA_VISIBLE_DEVICES="$GPU" python dao/finetune.py train.finetune_mode=${FINETUNE_MODE} \
...
for fold in {0..0};do
       ...
       echo "fold ${fold}"
       CUDA_VISIBLE_DEVICES="$GPU" python dao/finetune.py train.finetune_mode=${FINETUNE_MODE} data=$DATA data.fold=$fold \
