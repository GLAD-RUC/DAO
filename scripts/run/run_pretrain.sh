...
CUDA_VISIBLE_DEVICES='0,1,2' python dao/run.py expname=DAO_G_Stage1 \
...
CUDA_VISIBLE_DEVICES='0,1,2' python dao/run.py expname=DAO_G_Stage2 \
    data=pretrain_daog_data data.train_max_epochs=$EPOCHS train.pretrain=True \
    optim.lr_scheduler_cos.T_0=$EPOCHS optim.optimizer.lr=0.0001  optim.warm_epochs=50\
    +load_state_dict_only=True +ckpt_path=$CKPT\
...
echo $EPOCHS
CKPT=''
CUDA_VISIBLE_DEVICES='0,1' python dao/run.py expname=DAO_P \
