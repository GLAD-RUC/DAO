...
class CSPDiffusion(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.use_time = self.hparams.use_time 
        latent_dim = self.hparams.latent_dim + self.hparams.time_dim if self.use_time else self.hparams.latent_dim
                
        self.decoder = hydra.utils.instantiate(self.hparams.decoder, latent_dim = latent_dim, use_time=self.use_time, pred_scalar=self.hparams.pred_scalar, _recursive_=False)
    ...
    def forward(self, batch):
        ...
        if not self.use_time or self.keep_lattice:
            input_lattice = lattices

        if self.decoder.pred_scalar:
            pred_scalar = self.decoder(time_emb, batch.atom_types, input_frac_coords, input_lattice, batch.num_atoms, batch.batch)
            tar_scalar = batch.y
    ...
    def compute_stats(self, output_dict, prefix):
        ...
        log_dict = {
            f'{prefix}_loss': loss,
            f'{prefix}_scalar_loss': loss_scalar,
...
class CSPDenoise(BaseModule):
    ...
    def forward(self, batch):
        ...
        loss_lattice = F.mse_loss(pred_l, lattices)
        loss_coord = F.mse_loss(pred_x, tar_x)
